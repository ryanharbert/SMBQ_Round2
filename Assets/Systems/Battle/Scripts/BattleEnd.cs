...
public class BattleEnd : MonoBehaviour
{
    ...
    void RaidPointsSetup()
    {
        ...
        if (Data.instance.CurrentPvPEventTier(Mathf.RoundToInt(increasingNumber)) == -1)
        {
            rankText.gameObject.SetActive(true);
            rankText.text = "Rank #" + rank;
            pointsDisplayType = EventRewardDisplayType.TiersCompleted;
        }
        else if (Data.instance.CurrentPvPEventTier(Mathf.RoundToInt(increasingNumber)) == Data.instance.CurrentPvPEventTier(Data.instance.currency.asyncPoints))
        {
            rankText.gameObject.SetActive(false);
            pointsForNextReward.gameObject.SetActive(true);
            pointsForNextReward.text = "Earn " + (Data.instance.pvpEvent.Point[Data.instance.CurrentPvPEventTier(Data.instance.currency.asyncPoints)].Req - Data.instance.currency.asyncPoints) + " more trophies for next reward";
        ...
        else
        {
            ...
            tierRewardQuantityText.text = "+" + Data.instance.pvpEvent.Point[Data.instance.CurrentPvPEventTier(Data.instance.currency.asyncPoints - arenaPointIncrease)].Amt;
			if(Data.instance.pvpEvent.Point[Data.instance.CurrentPvPEventTier(Data.instance.currency.asyncPoints - arenaPointIncrease)].Rew == "GO")
			{
				Data.instance.currency.gold += Data.instance.pvpEvent.Point[Data.instance.CurrentPvPEventTier(Data.instance.currency.asyncPoints - arenaPointIncrease)].Amt;
				tierRewardCurrencyImg.sprite = Resources.Load<Sprite>("CurrencyOffers/GoldPile");
			}
			else
			{
				Data.instance.currency.stars += Data.instance.pvpEvent.Point[Data.instance.CurrentPvPEventTier(Data.instance.currency.asyncPoints - arenaPointIncrease)].Amt;
    ...
    void RaidPointIncreaseDisplay(int target, int increaseAmount)
    {
        ...
        {
            currencyAmountText.text = Mathf.Floor(increasingNumber) + " / " + Data.instance.pvpEvent.Point[Data.instance.CurrentPvPEventTier(Mathf.RoundToInt(increasingNumber))].Req;
        }
        else
        {
            if (Mathf.Floor(increasingNumber) >= Data.instance.pvpEvent.Point[Data.instance.CurrentPvPEventTier(Data.instance.currency.asyncPoints - arenaPointIncrease)].Req)
            {
                if (Data.instance.CurrentPvPEventTier(Data.instance.currency.asyncPoints) == -1)
                {
                    pointsDisplayType = EventRewardDisplayType.TiersCompleted;
                }
                else
